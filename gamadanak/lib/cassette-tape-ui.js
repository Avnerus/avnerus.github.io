...
function ControlHandler(tapeUIDOM, soundObject) {
  ...
  events = {
    ...
    click: function(e) {
      ...
      if (typeof eventMap[className] == 'undefined') {
          window.emitter.emit('pausePressed',{} );
      } else {
        return events.mouseup(e);
      }
    }
  };
  ...
  eventMap = {
    'play': function(e) {
      window.emitter.emit('playPressed',{} );
      DoFullScreen();
     // soundObject.play();
    },
    'rew': function() {
      // rewind
      window.emitter.emit('rewindPressed',{} );
/*      var newPosition = Math.max(0, soundObject.position - soundObject.duration * rewind_ffwd_offset);
    ...
    'ffwd': function() {
      window.emitter.emit('ffwdPressed',{} );
      // fast-forward
/*      var newPosition;
      if (soundObject.duration) {
    ...
    'stop': function() {
     // equivalent to digital pause
      window.emitter.emit('stopPressed',{} );
      //soundObject.pause();
    }
  };
...
function init() {
  ...
  window.emitter.on('tapeStart', function(message){
      genericStart();
  }); 
  window.emitter.on('tapeStop', function(message){
      genericStop();
  }); 
